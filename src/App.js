...
class App extends Component{
    ...
    addTodo = () => {
        ...
        this.alert('Add successfully', 'primary')
    };
    editingTodo = (index) => {
        let todo = this.state.todos[index];
        this.setState({editing: true, newItem: todo.name, indexItem: index})
    ...
    updateTodo = () => {
        ...
        todos[this.state.indexItem].name = this.state.newItem;
        this.setState({
            todos: todos,
            editing: false

        });
        this.alert('Update successfully', 'success')
    };
    render() {
        return (
            <div>
                ...
                <div className="App">
                    <div className="container">
                        ...
                        />
                        <button
                            className="btn-info mb-3 form-control" onClick={this.state.editing? this.updateTodo : this.addTodo}>
                        ...
                        <ul className="list-group">
                            {this.state.todos.map((item, index)=>(
                                <li className="list-group-item" key={item.id}>
                                    <button className='btn btn-info mr-4' onClick={() => this.editingTodo(index)}>update</button>
                                    {item.name}
                                    <button className='btn btn-danger ml-4' onClick={() => this.onDelete(index)}>delete</button>
                                </li>
                            ))}
                        </ul>}
                    </div>
